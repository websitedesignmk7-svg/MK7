...
// Replace the entire div content for flex gap-4 inside hero
html = html.replace(/<div class="flex gap-4">[\s\S]*?<a href="#menu"/, `<div class="flex gap-4">\n${socialIcons}\n</div>\n                    </div>\n                </div>\n            </div>\n            <a href="#menu"`);

// Gallery
if (salonPics.length > 0) {
  const carouselHtml = salonPics.map(pic => `<div class="carousel-item"><img src="${pic}" alt="Salon image"></div>`).join('\n');
  
  // Find <div class="carousel-track"> and replace until next </div>
  const trackStart = html.indexOf('<div class="carousel-track">');
  const sectionEnd = html.indexOf('</section>', trackStart);
  const trackPart = html.substring(trackStart, sectionEnd);
  const replacedPart = trackPart.replace(/<div class="carousel-track">[\s\S]*?<\/div>\s*<\/div>/, `<div class="carousel-track">\n${carouselHtml}\n</div>\n        </div>`);
  
  html = html.substring(0, trackStart) + replacedPart + html.substring(sectionEnd);
}
...
if (menuRaw) {
  ...
  if (menuItems) {
    ...
    `;
    const menuStart = html.indexOf('<div class="grid md:grid-cols-2 gap-12">');
    const menuEnd = html.indexOf('</section>', menuStart);
    html = html.substring(0, menuStart) + newMenuHtml + '\n</div>\n' + html.substring(menuEnd);
  }
}
...
fs.writeFileSync('C:/Users/Mouad/Desktop/MK7/Exotic_barber_shop/index.html', html);
